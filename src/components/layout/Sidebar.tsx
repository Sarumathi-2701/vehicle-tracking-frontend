...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 left-0 z-50 h-screen bg-[#0B132B] border-r border-slate-800 text-slate-300 transition-all duration-300 flex flex-col justify-between shadow-2xl ${
      ...
      >
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {(sidebarOpen || mobileSidebarOpen) && item.badgeCount && (
                  ...
                  {!sidebarOpen && !mobileSidebarOpen && (
                    <div className="hidden lg:block absolute left-full ml-3 px-2.5 py-1 bg-slate-900 border border-slate-700 text-white text-xs rounded-md shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                      {item.label}
                    </div>
                  )}
                </NavLink>
        ...
        <div className="p-3 border-t border-slate-800/80">
          ...
          >
            ...
            {(sidebarOpen || mobileSidebarOpen) && <Radio className="w-3.5 h-3.5 text-emerald-400" />}
          </div>
        </div>
      </aside>
    </>
